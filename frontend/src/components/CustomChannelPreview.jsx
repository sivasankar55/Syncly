...
const CustomChannelPreview = ({ channel, setActiveChannel, activeChannel }) => {
  const isActive = activeChannel && activeChannel.id === channel.id;// check if the channel is active
  const isDM = channel.data.member_count === 2 && channel.data.id.includes("user_"); // check if the channel is a DM

  if (isDM) return null; 

  const unreadCount = channel.countUnread();// get the unread count
