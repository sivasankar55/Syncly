...
const UsersList = ({ activeChannel }) => {
  ...
  const [_, setSearchParams] = useSearchParams();// to set the search params

  const fetchUsers = useCallback(async () => {
    if (!client?.user) return; // to check if the user is logged in
    ...
    const response = await client.queryUsers(
     ...
     const usersOnly = response.users.filter((user) => !user.id.startsWith("recording-"));// to remove the recording 
  ...
  });// it does the react query the data is fresh for 5 minutes so in that 5 minutes the data is not fetched again

  const startDirectMessage = async (targetUser) => {
    if (!targetUser || !client?.user) return;

    try {
     //stream not  allows channelId to be longer than 64 characters
